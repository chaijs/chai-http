...
export default function (chai, util) {
  ...
  const { inspect, transferFlags } = util;
  ...
  function getHeader(obj, key) {
    ...
    throw new Error(`could not get header from object ${ obj }`);
  }
  ...
  Assertion.addMethod('status', function assertStatus(code) {
  ...
  Assertion.addMethod('header', function assertHeader(key, value) {
    ...
    if (arguments.length < 2) {
      ...
      this.assert(
        value.test(header)
        , `expected header '${ key }' to match ${ value } but got ${ inspect(header) }`
        , `expected header '${ key }' not to match ${ value } but got ${ inspect(header) }`
        , value
        , header
      );
    } else {
      this.assert(
        header === value
        , `expected header '${ key }' to have value ${ value } but got ${ inspect(header) }`
  ...
  Assertion.addProperty('headers', function assertHeaders() {
    this.assert(
  ...
  Assertion.addProperty('ip', function assertIP() {
    this.assert(
      isIP(this._obj)
  ...
  function checkContentType(name) {
    ...
    Assertion.addProperty(name, function assertContentType() {
      new Assertion(this._obj).to.have.headers;
      const contentType = getHeader(this._obj, 'content-type');
      const ins = inspect(contentType) === 'undefined' ?
        'headers' :
        inspect(contentType);

      this.assert(
        contentType && contentType.indexOf(val) !== -1
  ...
  Assertion.addProperty('redirect', function assertRedirect() {
    const redirectCodes = [ 301, 302, 303, 307, 308 ];
    const { status, redirects } = this._obj;
  ...
  Assertion.addMethod('redirectTo', function assertRedirectTo(destination) {
  ...
  Assertion.addMethod('param', function assertParam(...args) {
    const assertion = new Assertion();
    transferFlags(this, assertion);
    assertion._obj = parseQuery(parseUrl(this._obj.url).query);
  ...
  Assertion.addMethod('cookie', function assertCookie(key, value) {
