...
import { agent as Agent, Request } from 'superagent';
...
export default function request(app) {
  ...
  let server = (typeof app === 'function') ?
  ...
  obj.keepOpen = function keepOpen() {
    keepServerOpen = true;
    return this;
  };
  obj.close = function close(onClose) {
    if (server && server.close && keepServerOpen === false) {
      server.close(onClose);
    }
    return this;
  };
  methods.forEach((method) => {
    obj[method] = (path) => new Test(server, method, path)
      .on('end', () => {
        obj.close();
      });
  });
...
TestAgent.prototype.close = function close(onClose) {
  if (this.app && this.app.close) {
    this.app.close(onClose);
  }
  return this;
};
...
methods.forEach((method) => {
  TestAgent.prototype[method] = function testAgentMethod(url) {
